...
class NormExperiment(Settings):
        ...
        if self.pred_len == 1:
            ...
            self.metrics = MetricCollection(
                metrics={
                    "mse": MeanSquaredError(),
                    "mae": MeanAbsoluteError(),
                    #"mape": MaskedMAPE(null_val=0),###20250110
    ...
    def _init_n_model(self):
        ...
        elif "SAN" in self.norm_type: #self.norm_type == 'SAN':
    ...
    def _setup_run(self, seed):
        ...
        self.run_save_dir = os.path.join(
            ...
            f"{self.norm_type}_{self.norm_config}_{seed}",
        )
        '''
        self.run_save_dir = os.path.join(
            self.save_dir,
    ...
    def _train(self):
        with torch.enable_grad(), tqdm(total=self.train_steps,position=0, leave=True) as progress_bar:
            ...
            for i, (
                ...
                # 
                if  "SAN" in self.model.nm.__class__.__name__: #isinstance(self.model.nm, SAN):
                ...
                import math
                if math.isnan(loss.item()):######check nan
                    print("true", true)
                    print("pred", pred)
                    print("loss", self.loss_func(pred, true))
                    print("nmloss",self.model.nm.loss(true))
                    print("NaN detected in loss.item()!")
                    raise ValueError("NaN detected in loss.item()!")
    ...
    def run(self, seed=42) -> Dict[str, float]:
        ...
        model_name = str(self.model_type)+str(self.norm_type)+str(self.norm_config)
        if model_name+list(result.keys())[0] not in result_df.columns:
